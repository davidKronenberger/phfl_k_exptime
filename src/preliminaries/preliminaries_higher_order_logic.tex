...
\colon \odot).\, \neg a(x_1,y_1)\big) \\&
\underset{a\in\Sigma}{\bigvee} \big(\exists (y_1 \colon \odot).\, \exists (y_2 \colon \odot).\, a(x_1, y_1) \wedge a(x_2, y_2) \wedge X(y_1, y_2)\big)
\end{align*}
...
\begin{example}
Let $\Phi(X, x_1, x_2)$ be the HO$^2$ formula and $\mathcal{T}$ the LTS of Example~\ref{example:ho_lfp} then $PFP(F_\Phi^T)$ describes also the set of all those pairs of states $(q, p)$ in $\mathcal{T}$ such that $q \not\sim p$ holds. The reason is that $F_\Phi^T$ is monotone. With $[PFP\; \Phi(X, x_1, x_2)](q, p)$ we can check if $q \not\sim p$, where $q, p \in Q$. 
\end{example}